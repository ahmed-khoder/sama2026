...
export async function GET(request: NextRequest) {
  try {
    ...
    allBroken.push(
      ...(await scanModel("HeroSlide", heroSlides, [
        "imageDesktopAr",
        "imageDesktopEn",
        "imageMobile",
      ])),
    ...
    allBroken.push(
      ...(await scanModel("AboutHeroSlide", aboutSlides, [
        "imageDesktopAr",
        "imageDesktopEn",
        "imageMobile",
      ])),
    ...
    allBroken.push(
      ...(await scanModel("AboutWhyImage", aboutWhyImages, ["url"])),
    );

    // ── Service ───────────────────────────────────────────────
    const services = await prisma.service.findMany();
    allBroken.push(
      ...(await scanModel("Service", services, ["image", "imageMobile"])),
    );

    // ── ServicesHeroSlide ─────────────────────────────────────
    const svcSlides = await prisma.servicesHeroSlide.findMany();
    allBroken.push(
      ...(await scanModel("ServicesHeroSlide", svcSlides, [
        "imageDesktopAr",
        "imageDesktopEn",
        "imageMobile",
      ])),
    ...
    // ── BlogPost (Hero + Content Blocks) ──────────────────────────────
    const blogs = await prisma.blogPost.findMany();
    // Scan main image
    allBroken.push(...(await scanModel("BlogPost", blogs, ["image"])));

    // Deep scan inline content imagery via Regex
    const imgRegex = /<img[^>]+src="([^">]+)"/g;
    for (const blog of blogs) {
      const arMatches = [...(blog.contentAr || "").matchAll(imgRegex)];
      const enMatches = [...(blog.contentEn || "").matchAll(imgRegex)];
      const urls = [...arMatches, ...enMatches]
        .map((m) => m[1])
    ...
    allBroken.push(
      ...(await scanModel("CareersHeroSlide", careerSlides, [
        "imageDesktopAr",
        "imageDesktopEn",
        "imageMobile",
      ])),
    ...
    allBroken.push(
      ...(await scanModel("Accreditation", accreditations, ["logo"])),
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (searchParams.get("fix") !== "true") {
      return NextResponse.json(
        { error: "Pass ?fix=true to apply fixes" },
        { status: 400 },
      );
    }
    ...
    // Deep scan for fix
    const imgRegexFix = /<img[^>]+src="([^">]+)"/g;
    for (const blog of blogs) {
      const arMatches = [...(blog.contentAr || "").matchAll(imgRegexFix)];
      const enMatches = [...(blog.contentEn || "").matchAll(imgRegexFix)];
      const urls = [...arMatches, ...enMatches]
        .map((m) => m[1])
        .filter((url) => url && url.startsWith("/"));
    ...
    allBroken.push(
      ...(await scanModel("CareersHeroSlide", careerSlides2, [
        "imageDesktopAr",
        "imageDesktopEn",
        "imageMobile",
      ])),
    ...
    // ── Accreditation ─────────────────────────────────────────
    const accreditations2 = await prisma.accreditation.findMany();
    allBroken.push(
      ...(await scanModel("Accreditation", accreditations2, ["logo"])),
    );
    ...
    for (const b of allBroken) {
      try {
        ...
        console.error(
          `Failed to fix ${b.table}.${b.field} on ${b.recordId}:`,
          err,
        );
      }
    }
...
function getModel(tableName: string): any {
  const map: Record<string, any> = {
    ...
    Accreditation: prisma.accreditation,
  };
