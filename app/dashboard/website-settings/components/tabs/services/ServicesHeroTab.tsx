...
import { Plus, Trash2, ChevronUp, ChevronDown, Monitor, Smartphone, Upload, Image as ImageIcon, RefreshCw, Settings, Layers, Type, Palette } from 'lucide-react';
import { SlideImageUploader } from '../../SlideImageUploader';
import { ServicesHeroSlide, ServicesHeroSettings } from '../../../types';
import { useConfirm } from '@/contexts/ConfirmContext';

interface ServicesHeroTabProps {
    ...
    onSettingsChange: (field: keyof ServicesHeroSettings, value: any) => void;
...
function ColorPickerField({
    ...
    return (
        <div className="p-3 bg-gray-50 dark:bg-slate-900 rounded-xl space-y-2">
            ...
            <div className="flex items-center gap-3">
                <input
                    ...
                    className="w-12 h-12 rounded-lg cursor-pointer border-2 border-white dark:border-slate-700 shadow"
                />
                <div className="flex items-center gap-2 p-2 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-600 flex-1">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent border-none text-gray-900 dark:text-white font-mono text-sm uppercase focus:outline-none"
                        maxLength={6}
                    />
                </div>
            </div>
...
export function ServicesHeroTab({
    slides,
    slidesLoading,
    settings,
    isRTL,
    ...
    const handleImageChange = async (slideId: string, file: File, type: 'imageDesktopAr' | 'imageDesktopEn' | 'imageMobile') => {
        ...
        const path = await uploadImage(file, 'services-hero');
        if (path) {
                                ...
                                <div key={slide.id} className="p-6 border border-gray-200 dark:border-slate-700 rounded-2xl bg-gray-50 dark:bg-slate-900 space-y-6">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => onDeleteSlide(slide.id)} className="p-2 bg-red-50 dark:bg-red-900/30 text-red-600 hover:bg-red-100 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </div>
