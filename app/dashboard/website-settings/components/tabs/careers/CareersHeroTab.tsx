...
import { CareersHeroSlide, CareersHeroSettings } from '../../../types';
import { useConfirm } from '@/contexts/ConfirmContext';

interface CareersHeroTabProps {
    slides: CareersHeroSlide[];
...
export function CareersHeroTab({
    slides,
    settings,
    slidesLoading,
    isRTL,
    ...
    const handleImageChange = async (slideId: string, file: File, type: 'imageDesktopAr' | 'imageDesktopEn' | 'imageMobile') => {
        ...
        const path = await uploadImage(file, 'careers-hero');
        if (path) {
